...
for j in range(5):
	...
	for k in requests:
		for i in threads:
			string = "sysbench --test=oltp --num-threads=" + str(i) + " --max-requests=" + str(k) + " --db-driver=mysql --mysql-user=root --mysql-password=root --mysql-table-engine=ndbcluster --mysql-db=sakila run"
