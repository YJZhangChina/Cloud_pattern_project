...
Figure \ref{fig:Thread Schema} depicts the life cycle of a thread in the Java \texttt{Thread} class. After creating a new instance of the\texttt{Thread} class, we call \texttt{start()} to make the thread runnable, which means the thread is ready to run the given tasks. We then call our overridden method \texttt{run()} to execute our scenario in each thread (\emph{i.e.,} for each client). After executing the code in the \texttt{run} method, the thread can either wait or be terminated.  \\ 
