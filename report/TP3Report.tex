...
The Gatekeeper pattern aims at keeping sensitive information safe \cite{gk_microsoft}. We use this pattern to prevent malicious users from injecting wrongful SQL commands to our database, which could compromise the application. The basic concept is to apply a dedicated machine that filters the requests and prevent direct access to the database. End users can only see the gatekeeper and cannot see the functionalities or gain unrestrained access to sensitive data. 
