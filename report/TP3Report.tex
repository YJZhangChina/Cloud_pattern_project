...
We use Java Database Connectivity (JDBC) to send MySQL requests from the master node to slave nodes. We configure the port 3306 for the MySQL communication among the 4 data nodes.
...
In the rest of this section, we elaborate three key points of the implementation of data nodes' configuration, trusted host's configuration and implementation, and gatekeeper's implementation. Our source code in Java can be found at: \url{???the-link???}.
