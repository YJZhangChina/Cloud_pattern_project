...
The Gatekeeper pattern aims at keeping sensitive information safe \cite{gk_microsoft}. We use this pattern to prevent malicious user from injecting wrongful SQL commands to our database, which could compromise the application. The basic concept is to apply a dedicated machine that filters the requests and prevent direct access to the database. End users can only see the gatekeeper and cannot see the functionalities or gain unrestrained access to sensitive data. 
...
The main goal of a multi-threaded application is to allow the application to run mulitple processes at a given time.  In the case of our application, the goal of the threads is to send multiple requests at the same time. This way, we are able to simulate very high traffic from a single machine. 

The ClientThreadMQ.java inherits the Java Thread class. This built-in class allows the application to have several threads of executions running concurently ~\cite{threads_java}. We call the start() method from our ClientThreadMQ class to start the desired number of threads. Here, one thread represents one client. Once our threads are started, we use our overwritten version of the run() method to start the processes. Run() will execute the scenario stated in the TP instruction. Each thread sends five \texttt{SELECT} queries, waits five seconds, sends another 100 \texttt{SELECT} queries, waits five seconds, and ends by sending 5 \texttt{INSERT} queries. 
