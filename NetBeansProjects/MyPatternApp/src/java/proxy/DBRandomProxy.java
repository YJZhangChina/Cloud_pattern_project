...
public class DBRandomProxy implements InvocationHandler {
            ...
            if (((String) args[0]).toLowerCase().startsWith("select")) {
                try {
                    if(slaves.size() == 1){
                        ...
                        int dbIndex = rand.nextInt(slaves.size());
                        ...
                        return SlaveDBConnection.getDbCon(slaves.get(dbIndex)).query((String) args[0]);
                    }
